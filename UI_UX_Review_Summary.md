...
# Daily UI/UX Review: hermy-hq Pages — 2026-09-26

## Executive Summary

**Status:** ✅ COMPLETED - 5 Issues Fixed

**Scope:** `src/app/page.tsx`, `src/app/agents/page.tsx`, `src/components/ui/metric-card.tsx`, `src/components/OfficeView.tsx`, `src/components/agent-proposals-widget.tsx`, `src/components/decision-dashboard-widget.tsx`

---

## 🔧 FIXES APPLIED

### 1. MetricCard with href — focus-visible ring restored ⭐ HIGH
**File:** `src/components/ui/metric-card.tsx:136`

**Problem:** `<a>` wrapper lacked `panel-interactive` class; inner `<div>` had it but focus landed on the anchor, making keyboard focus invisible on linked metric cards (GitHub, Homelab, Agent Tokens, OmniRoute, FreeLLM, X Followers).

**Fix:** Added `panel-interactive` to the anchor className.
```tsx
className={`block ${fill ? "h-full" : ""} panel-interactive`}
```

### 2. OfficeView AgentDesk — keyboard accessible ⭐ HIGH
**File:** `src/components/OfficeView.tsx:254`

**Problem:** Agent desks were interactive visual elements with hover animations but had no `tabIndex`, `role`, or focus styles.

**Fix:** Added `tabIndex={0}`, `role="button"`, `aria-label`, `onKeyDown` handler, and `focus-visible:outline-2 focus-visible:outline-[var(--accent)] focus-visible:outline-offset-2` to the desk tile.

### 3. AgentProposalsWidget link — focus-visible added ⭐ MEDIUM
**File:** `src/components/agent-proposals-widget.tsx:234`

**Problem:** "Review all on Agents floor" link had hover transition only, no focus-visible indicator.

**Fix:** Added `focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[var(--accent)]`.

### 4. AgentCard focus ring — stronger on dark surface ⭐ MEDIUM
**File:** `src/app/agents/page.tsx:79`

**Problem:** Global CSS `box-shadow: 0 0 0 2px var(--accent)` on `panel-interactive` is low-contrast on the dark surface with transform.

**Fix:** Added explicit `focus-visible:outline-2 focus-visible:outline-[var(--accent)] focus-visible:outline-offset-[-2px]` to override the subtle box-shadow.

### 5. DecisionDashboardWidget "View all" button — elevated on focus ⭐ LOW
**File:** `src/components/decision-dashboard-widget.tsx:207`

**Problem:** Tertiary action used `text-[var(--text-3)]`; focused state didn't elevate visual weight enough.

**Fix:** Added `hover:bg-[var(--surface-2)] focus-visible:bg-[var(--surface-2)]` so the button background lifts on focus/hover.
...
- `graphify update .` — graph rebuilt (1810 nodes, 2576 edges)
- All changes are additive styling only; no behavioral or data changes